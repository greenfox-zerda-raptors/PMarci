...
public class Workshop07{
    ...
    static void myMethod(int inNumber) throws MySevenException// something should appear here before
    {
        if (inNumber == 7) {// do that thing that chucks the reprobate out
            throw new MySevenException("oh shit nigga whachu doin");
        }
            System.out.println("Nice number");
        return;
    }

    public static class MySevenException extends Exception {
        public MySevenException(String message) {
            super(message);
        }
    }
